...
static uint8_t config_is_valid(const config_t* config)
{
    ...
    // Version check
    if (config->version > 2) return 0;  // Only versions 0-2 supported

    // Soft limit checks
    if (config->soft_limit_low_mm >= config->soft_limit_high_mm) return 0;
    if (config->soft_limit_high_mm > 1000) return 0;  // Reasonable max

    // Speed checks
    if (config->min_speed_hz < 1 || config->min_speed_hz > 10) return 0;
    if (config->max_speed_hz < 50 || config->max_speed_hz > 105) return 0;
    if (config->min_speed_hz >= config->max_speed_hz) return 0;

    // VFD ramp time checks (in 0.1s units)
    if (config->vfd_acc_time_100ms < 2 || config->vfd_acc_time_100ms > 20) return 0;
    if (config->vfd_dec_time_100ms < 2 || config->vfd_dec_time_100ms > 20) return 0;
    ...
    // Axis count check
    if (config->axis_count != 3) return 0;  // Only 3-axis supported
...
{
    if (!config || !config_is_valid(config)) return 0;
...
{
    if (!config || storage_size == 0) return 0;

    std::memcpy(config, storage_buffer, sizeof(config_t));

    // Verify checksum
    uint32_t calculated = config_calculate_checksum(config);
    if (calculated != config->checksum) return 0;
...
void test_load_fails_on_corrupt_checksum(void)
{
    ...
    uint32_t* stored_checksum = (uint32_t*)(&storage_buffer[sizeof(config_t) - sizeof(uint32_t)]);
