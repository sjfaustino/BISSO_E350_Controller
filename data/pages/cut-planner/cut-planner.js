...
if (typeof window.CutPlanner === 'undefined') {
    window.CutPlanner = {
        ...
        generateCornerDrilling(p) {
            const radius = p.holeDiameter / 2;
            const safeZ = 20; // Retract height above surface

            // Map speed to feedrate (F1=slow, F2=medium, F3=fast)
            const speedMap = { slow: 1, medium: 2, fast: 3 };
            const feedRate = speedMap[p.speed] || 2;

            // Calculate hole centers (inset by radius from cutout edges)
            // Origin is at bottom-left of cutout
            const holes = {
                'bottom-left': { x: radius, y: radius },
                'bottom-right': { x: p.cutoutX - radius, y: radius },
                'top-right': { x: p.cutoutX - radius, y: p.cutoutY - radius },
                'top-left': { x: radius, y: p.cutoutY - radius }
            };

            // Determine drilling sequence (clockwise from starting corner)
            const sequences = {
            ...
            const sequence = sequences[p.startCorner] || sequences['top-left'];
            ...
            for (let i = 0; i < sequence.length; i++) {
                const corner = sequence[i];
                const hole = holes[corner];

                lines.push(`; --- Hole ${i + 1}: ${corner.replace('-', ' ').toUpperCase()} ---`);

                // Move to hole position (relative from current position)
                const deltaX = hole.x - currentX;
                const deltaY = hole.y - currentY;

                if (i === 0) {
                    // First hole: assume we start at position (0,0)
                    lines.push(`G91              ; Relative mode for positioning`);
                    if (deltaX !== 0 || deltaY !== 0) {
                        lines.push(`G0 X${deltaX.toFixed(1)} Y${deltaY.toFixed(1)} F3  ; Move to first hole`);
                    }
                } else {
                    // Subsequent holes: rapid move
                    lines.push(`G0 X${deltaX.toFixed(1)} Y${deltaY.toFixed(1)} F3  ; Move to hole`);
                }
                ...
                while (drilledDepth < p.depth) {
                    ...
                    if (p.dwellTime > 0 && drilledDepth < p.depth) {
                        lines.push(`G4 P${p.dwellTime.toFixed(1)}          ; Dwell for chip clearing`);
                    }
                }

                // Retract
                lines.push(`G0 Z${(p.depth + safeZ).toFixed(1)} F3    ; Retract to safe height`);
                lines.push('');
            }

            // Return to origin and finish
            lines.push('; --- Return to origin ---');
            lines.push(`G0 X${(-currentX).toFixed(1)} Y${(-currentY).toFixed(1)} F3  ; Return to start`);
            lines.push(`G0 Z${(-safeZ).toFixed(1)} F3             ; Lower to starting Z`);
            lines.push('');
            lines.push('M5               ; Drill OFF');
            lines.push('M9               ; Coolant OFF');
            lines.push('');
            lines.push('; Job complete - 4 corner holes drilled');
