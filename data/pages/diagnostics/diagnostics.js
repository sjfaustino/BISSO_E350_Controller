window.DiagnosticsModule = window.DiagnosticsModule || {
    updateInterval: null, trendInterval: null, spindleInterval: null, spindleData: [], spindlePaused: !1, spindleMaxPoints: 120, tachometerInterval: null, rs485Interval: null, setNA(t, e = "") { t && (t.innerHTML = '<span class="value-na">n/a</span>' + e) }, updateHeaderNA(t, e, n) { const a = document.getElementById(t); a && (a.innerHTML = n ? e : `${e} <span class="value-na" style="font-size: 0.8em; margin-left: 8px;">(N/A)</span>`) }, init() { console.log("[Diagnostics] Initializing"), window.addEventListener("state-changed", () => this.onStateChanged()), this.setupEventListeners(), this.loadInitialData(), this.updateInterval = setInterval(() => this.loadIOStatus(), 2e3), this.loadTrendData(), this.trendInterval = setInterval(() => this.loadTrendData(), 5e3), this.loadSpindleData(), this.spindleInterval = setInterval(() => this.loadSpindleData(), 500), this.loadIODiagnostics(), this.ioDiagInterval = setInterval(() => this.loadIODiagnostics(), 2e3), this.startTachometerPolling(), this.loadBootLog(), this.updateRS485Status(), this.rs485Interval = setInterval(() => this.updateRS485Status(), 3e3) }, setupEventListeners() { document.getElementById("refresh-io-btn")?.addEventListener("click", () => { this.loadIOStatus() }), document.getElementById("clear-faults-btn")?.addEventListener("click", () => { this.clearFaults() }), document.getElementById("refresh-trends-btn")?.addEventListener("click", () => { this.loadTrendData() }), document.getElementById("refresh-io-diag-btn")?.addEventListener("click", () => { this.loadIODiagnostics() }), document.getElementById("refresh-bootlog-btn")?.addEventListener("click", () => { this.loadBootLog() }), document.getElementById("delete-bootlog-btn")?.addEventListener("click", () => { this.deleteBootLog() }), document.getElementById("spindle-graph-pause")?.addEventListener("click", () => { this.spindlePaused = !this.spindlePaused; const t = document.getElementById("spindle-graph-pause"); t && (t.textContent = this.spindlePaused ? "Resume" : "Pause") }) }, loadBootLog() { const textarea = document.getElementById("bootlog-content"); const sizeEl = document.getElementById("bootlog-size"); if (!textarea) return; if ("file:" === window.location.protocol) { textarea.value = "(Mock mode - boot log not available)"; if (sizeEl) sizeEl.textContent = "0"; return } fetch("/api/logs/boot").then(r => r.text()).then(text => { textarea.value = text || "(Empty)"; if (sizeEl) sizeEl.textContent = text.length.toString() }).catch(err => { console.warn("[Diagnostics] Boot log fetch failed:", err); textarea.value = "(Failed to load boot log)"; if (sizeEl) sizeEl.textContent = "--" }) }, deleteBootLog() { if ("file:" === window.location.protocol) { AlertManager.add("Cannot delete in mock mode", "warning", 2e3); return } if (!confirm("Delete boot log?")) return; fetch("/api/logs/boot", { method: "DELETE" }).then(r => r.json()).then(data => { if (data.success) { AlertManager.add("Boot log deleted", "success", 2e3); this.loadBootLog() } else { AlertManager.add("Failed to delete boot log", "error", 3e3) } }).catch(err => { console.error("[Diagnostics] Delete boot log failed:", err); AlertManager.add("Failed to delete boot log", "error", 3e3) }) }, loadInitialData() { if ("file:" === window.location.protocol) return console.log("[Diagnostics] Mock mode - using simulated data"), void this.simulateDiagnosticData(); this.loadIOStatus(), this.loadFaultLog() }, onStateChanged() {
    ...
    loadIOStatus() { "file:" !== window.location.protocol && fetch("/api/io/status").then(t => t.json()).then(t => { t.success && (this.setIOIndicator("io-estop", t.estop, !0), this.setIOIndicator("io-door", t.door), this.setIOIndicator("io-probe", t.probe), this.setIOIndicator("io-limit-x", t.limit_x, !0), this.setIOIndicator("io-limit-y", t.limit_y, !0), this.setIOIndicator("io-limit-z", t.limit_z, !0), this.setIOIndicator("io-spindle", t.spindle_on), this.setIOIndicator("io-coolant", t.coolant_on), this.setIOIndicator("io-vacuum", t.vacuum_on), this.setIOIndicator("io-alarm", t.alarm_on, !0)) }).catch(t => { console.warn("[Diagnostics] Failed to load I/O status:", t) }) }, setIOIndicator(t, e, n = !1) { const a = document.getElementById(t); a && (a.classList.remove("on", "off", "alarm"), e ? a.classList.add(n ? "alarm" : "on") : a.classList.add("off")) }, loadFaultLog() { "file:" !== window.location.protocol && fetch("/api/faults").then(t => { if (!t.ok) throw new Error(`HTTP error! status: ${t.status}`); return t.json() }).then(t => { this.updateFaultDisplay(t.faults || []) }).catch(t => { console.warn("[Diagnostics] Failed to load fault log:", t); const e = document.getElementById("fault-list"); e && (e.innerHTML = `<div class="log-entry log-error"><span>Error loading fault log: ${t.message}</span></div>`) }) }, updateFaultDisplay(t) {
