...
bool spindleMonitorUpdate(void) {
    ...
    switch (poll_state) {
        case POLL_STATE_IDLE: {
            ...
            if ((now - monitor_state.last_poll_time_ms) < monitor_state.poll_interval_ms) {
        ...
        case POLL_STATE_PARSE_RESPONSE: {
            ...
            if (jxk10ModbusReceiveResponse()) {
                ...
                if (monitor_state.read_count == 1) {
                    ...
                    monitor_state.current_average_amps = (monitor_state.current_average_amps * 0.8f) + (current * 0.2f);
                }
...
void spindleMonitorPrintDiagnostics(void) {
    ...
    if (monitor_state.shutdown_count > 0) {
        Serial.printf("Last Shutdown:       %lu ms ago @ %.1f A\n",
