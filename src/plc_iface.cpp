...
// Fix - Shadow Register Dirty Flag (Mutex Timeout Handling)
...
static bool plc_in_transaction = false; // I2C Performance (Batching)
...
static bool plcWriteI2C(uint8_t address, uint8_t data, const char *context) {
 ...
 // Fix - Clear dirty flag on successful I2C write
 ...
 // I2C write failed - shadow register might be out of sync
 if (address == ADDR_Q73_OUTPUT || address == ADDR_Q73_AUX) {
 q73_shadow_dirty = true;
 }
...
result_t elboInit() {
 ...
 // Retry board detection with delay to handle slow power-on
 ...
 g_plc_hardware_present = false;
 // Don't return error — allow degraded boot but flag the failure
 }
 ...
 return board_detected ? RESULT_OK : RESULT_ERROR_HARDWARE;
}
...
void plcSetAxisSelect(uint8_t axis) {
 ...
 // Support multi-axis selection for coordinated motion
...
// Read current speed profile from shadow register
...
void plcPrintDiagnostics() {
 ...
 // Fix - Display shadow register health
...
// Fix - Shadow Register Health Monitoring
