...
#include "task_stall_detection.h" // Automatic task stall detection
...
#include "load_manager.h" // For loadManagerUpdate()
...
void taskMonitorFunction(void* parameter) {
 ...
 memoryMonitorInit();
 taskStallDetectionInit(); // Initialize task stall detection
 ...
 // Check for task stalls and log warnings
 taskStallDetectionUpdate();

 // 3.7. Load & Resource Management 
 ...
 // Telemetry collection moved to dedicated Core 0 task
