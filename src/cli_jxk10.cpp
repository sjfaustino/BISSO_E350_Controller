...
static void cmd_jxk10_addr(int argc, char** argv) {
    ...
    if (new_addr > 254) {
        logError("Invalid address: %u (max 254)", new_addr);
        return;
    }
    
    logPrintf("Changing address to %u...\n", new_addr);
    if (jxk10ModbusSetSlaveAddress(new_addr)) {
        logPrintf("Address changed successfully\n");
        logPrintf("POWER CYCLE REQUIRED for change to take effect!\n");
    } else {
        logError("Failed to change address");
    }
}
...
static void cmd_jxk10_enable() {
    ...
    logPrintf("Enabled in configuration (restart required)\n");
}

static void cmd_jxk10_disable() {
    configSetInt(KEY_JXK10_ENABLED, 0);
    logPrintf("Disabled in configuration (restart required)\n");
}
