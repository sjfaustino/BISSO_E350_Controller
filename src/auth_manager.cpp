 ...
 * @details Security hardening - CVSS 7.5 vulnerability fix
 */
...
void authInit() {
 ...
 if (is_first_boot) {
 // Generate random credentials on first boot
...
void authRecordFailedAttempt(const char* ip_address) {
 ...
 // Security - Only evict if the oldest entry is NOT actively rate limited (or expired)
