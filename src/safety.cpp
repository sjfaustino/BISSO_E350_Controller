...
#include "safety.h"
#include "altivar31_modbus.h" // VFD current-based stall detection
#include "axis_synchronization.h" // Per-axis motion validation
...
#include "vfd_current_calibration.h" // VFD current calibration
#include "system_tuning.h"
#include "rs485_device_registry.h" // RS485 Watchdog integration
...
void safetyUpdate() {
 ...
 // Supplementary VFD current-based stall detection
 ...
 // VFD Frequency Control Validation
 ...
 if (!alarm_active) {
 // Removed unsafe current_fault assignment (now done in safetyTriggerAlarm)
 char msg[64];
 ...
 // Axis Motion Quality Validation
 ...
 if (alarm_active) {
 // Wraparound-safe duration calculation
...
// + 5.10: Thread-safe alarm trigger with proper mutex error handling
// Added fault_type parameter to ensure thread-safe fault assignment
void safetyTriggerAlarm(const char *reason, safety_fault_t fault_type) {
 // CRITICAL FIX - Mutex must succeed for safety operations
 ...
 // Set current_fault under mutex protection (thread-safe)
...
// + 5.10: Safety Alarm Reset with proper mutex error handling
void safetyResetAlarm() {
 // CRITICAL FIX - Must acquire mutex for safe state access
 if (safety_state_mutex == NULL) {
