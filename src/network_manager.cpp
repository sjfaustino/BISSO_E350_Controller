...
#include "auth_manager.h" // For authVerifyCredentials (SHA-256)
...
void NetworkManager::initEthernet() {
 ...
 // Safety check for pin conflict on v3.1 (ESP32-S3)
 ...
 if (!use_dhcp) {
    char ip_str[32], gw_str[32], mask_str[32], dns_str[32];
    configGetStringSafe(KEY_ETH_IP, ip_str, sizeof(ip_str), "");
    configGetStringSafe(KEY_ETH_GW, gw_str, sizeof(gw_str), "");
    configGetStringSafe(KEY_ETH_MASK, mask_str, sizeof(mask_str), "255.255.255.0");
    configGetStringSafe(KEY_ETH_DNS, dns_str, sizeof(dns_str), "8.8.8.8");
...
void NetworkManager::init() {
 ...
 int ap_enabled = configGetInt(KEY_WIFI_AP_EN, 1); // Configurable AP

 if (ap_enabled) {
    char ap_ssid[33], ap_pass[65];
    configGetStringSafe(KEY_WIFI_AP_SSID, ap_ssid, sizeof(ap_ssid), "BISSO-E350-Setup");
    configGetStringSafe(KEY_WIFI_AP_PASS, ap_pass, sizeof(ap_pass), "BISSO-E350");

 WiFi.mode(WIFI_AP_STA);
 WiFi.softAP(ap_ssid, ap_pass);

 // Captive Portal DNS with safety checks
 ...
 if (ota_pw_changed) {
    char ota_md5[65];
    configGetStringSafe(KEY_OTA_PW_MD5, ota_md5, sizeof(ota_md5), "");
...
void NetworkManager::update() {
 ...
 // Check lockout BEFORE resetting auth state
 ...
 // Use auth_manager for credential verification (SHA-256)
