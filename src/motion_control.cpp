...
#include "motion_buffer.h" // Check buffer state in motionIsMoving
#include "motion_state_machine.h" // Formal state machine
#include "plc_iface.h"
#include "safety.h"
#include "operator_alerts.h"
#include "serial_logger.h"
#include "spindle_current_monitor.h" // Spindle current monitoring
#include "system_constants.h"
#include "task_manager.h"
#include "web_server.h"
#include "hardware_config.h"
#include "system_tuning.h"
#include "axis_utilities.h" // Centralized units
...
// Non-static to allow external access from motion_state_machine.cpp
...
// Uses Global Typed Cache (g_config) instead of local cache
...
// Coordinated Motion Support (C+T Mode)
...
result_t motionInit() {
 ...
 axes[i].accumulated_distance_units = (double)configGetFloat(d_key, 0.0f) * 1000.0;
...
void motionUpdate() {
 // Use spinlock for thread-safe global_enabled check
 portENTER_CRITICAL(&motionSpinlock);
 ...
 // Exponential backoff with safety escalation
 ...
 // Fix: Read consensus input OUTSIDE the mutex to prevent deadlock
 // if I2C bus hangs. This ensures motionUpdate doesn't hold the mutex for
 // 1000ms+.
 uint8_t current_axis = 255;

 // Use spinlock instead of mutex for consistent state protection
 ...
 // I2C Optimization - Read full input byte once per loop
 ...
 // Multi-key lookups replaced by atomic struct access
 ...
 // VFD IDLE SAFETY: If no axis is active, release VFD control back to PLC
 // This MUST stay in the motion loop for immediate safety response
...
int32_t motionGetPosition(uint8_t axis) {
 ...
 // Use spinlock to protect 32-bit position read (prevent torn reads)
 portENTER_CRITICAL(&motionSpinlock);
...
int32_t motionGetTarget(uint8_t axis) {
 ...
 // Use spinlock to protect 32-bit target read (prevent torn reads)
 portENTER_CRITICAL(&motionSpinlock);
...
float motionGetPositionMM(uint8_t axis) {
 ...
 // Use spinlock to protect position read
 portENTER_CRITICAL(&motionSpinlock);
...
// Use spinlock for thread-safe global_enabled read
...
// Use spinlock for thread-safe active_axis read
...
// Clear active axis (for state machine IDLE transition)
...
bool motionHome(uint8_t axis) {
 ...
 // Use spinlock for m_state access (consistent lock domain)
 portENTER_CRITICAL(&motionSpinlock);
 ...
 // Use formal state machine for state transitions
...
bool motionMoveAbsolute(float x, float y, float z, float a, float speed_mm_s) {
 ...
 // Use spinlock for m_state writes (consistent lock domain)
 portENTER_CRITICAL(&motionSpinlock);
 ...
 plcBeginTransaction();
 // Use precise Modbus velocities for coordinated motion
 ...
 // Use precise Modbus velocity for single-axis moves
...
void motionSetPLCSpeedProfile(speed_profile_t profile) {
 ...
 // If analog control enabled, also update VFD DAC frequencies
 if (dacIsAnalogEnabled()) {
...
bool motionStop() {
 ...
 // Use spinlock for m_state read (consistent lock domain)
 portENTER_CRITICAL(&motionSpinlock);
...
bool motionResume() {
 ...
 // Calculate precise frequency for resumption
...
void motionEmergencyStop() {
 // E-Stop Latency Monitoring (Recommendation)
 ...
 motionBuffer.clear();
 autoReportDisable(); // Disable auto-report on E-Stop
 lcdSleepWakeup(); // Wake display on E-Stop for visibility
 if (got_mutex)
 taskUnlockMutex(taskGetMotionMutex());
...
double motionGetAccumulatedDistance(uint8_t axis) {
 if (axis >= MOTION_AXES) return 0.0;
 // Read from axis struct — accumulated_distance_units is in mm
 portENTER_CRITICAL(&motionSpinlock);
 double dist = axes[axis].accumulated_distance_units;
 portEXIT_CRITICAL(&motionSpinlock);
 return dist;
}

// ============================================================================
// MAINTENANCE TASK (P0.1: Extracted from motionUpdate)
// ============================================================================
// Runs every 5 seconds at low priority. Handles:
// - Distance persistence to NVS (was blocking flash writes in 10ms motion loop)
// - Maintenance interval checks
// - Auto-report, LCD sleep, spindle monitoring
// None of these are time-critical and MUST NOT run in the motion loop.

static void motionMaintenanceTaskFunc(void* pvParameters) {
 (void)pvParameters;
 
 // Wait for system to stabilize after boot
 vTaskDelay(pdMS_TO_TICKS(5000));
 
 double last_saved_dist[3] = {0, 0, 0};
 for (int i = 0; i < 3; i++) {
 last_saved_dist[i] = motionGetAccumulatedDistance(i);
 }
 uint32_t last_nvs_save_ms = millis();
 
 while (1) {
 vTaskDelay(pdMS_TO_TICKS(5000)); // 5-second cycle
 
 // --- Periodic housekeeping (non-motion) ---
 autoReportUpdate();
 lcdSleepUpdate();
 spindleMonitorUpdate();
 
 // --- Distance Persistence & Maintenance ---
 uint32_t now = millis();
 bool should_save = ((uint32_t)(now - last_nvs_save_ms) > 3600000UL); // Hourly fallback
 
 if (!should_save) {
 for (int i = 0; i < 3; i++) {
 double current = motionGetAccumulatedDistance(i);
 if (fabs(current - last_saved_dist[i]) > 100000.0) { // 100m delta
 should_save = true;
 break;
 }
 }
 ...
 if (should_save) {
 // SAFETY: Only write NVS when motion is idle
 if (!motionIsMoving()) {
 bool changed = false;
 for (int i = 0; i < 3; i++) {
 double current = motionGetAccumulatedDistance(i);
 if (fabs(current - last_saved_dist[i]) > 1000.0) { // 1m resolution
 float dist_m = (float)(current / 1000.0);
 char d_key[16];
 snprintf(d_key, sizeof(d_key), "dist_%d_m", i);
 configSetFloat(d_key, dist_m);
 last_saved_dist[i] = current;
 changed = true;
 }
 }
 if (changed) {
 ...
 // --- Maintenance checks ---
 for (int i = 0; i < 3; i++) {
 float dist_m = (float)(motionGetAccumulatedDistance(i) / 1000.0);
 float maint_threshold = 50000.0f; // 50km
...
void motionStartMaintenanceTask() {
 xTaskCreate(
 motionMaintenanceTaskFunc,
 "MotionMaint",
 3072, // 3KB stack — only does config reads/writes and logging
 nullptr,
 1, // Priority 1 (lowest) — never preempts motion
 nullptr
 );
 logInfo("[MOTION] Maintenance task started (5s cycle, priority 1)");
}
