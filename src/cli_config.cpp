...
// or simpler: add a friend function or public dump utility in config_unified.cpp.
// For CLI simplicity here, we rely on the schema keys or just print what we know.
// Actually, config_unified.cpp has config_table but it's static. 
// Best practice: Add configUnifiedPrintAll() to config_unified.cpp and call it. 
// However, since I cannot modify config_unified.cpp in this specific turn (unless I output it too), 
// I will assume config_unified.cpp has a new function 'configUnifiedDump()' or we use a known list.
// WAIT -> I can modify config_unified.cpp if needed, but the user asked for CLI update.
// Let's implement 'dump' by iterating the known critical keys defined in schema or just allow config_unified.cpp 
// to expose a print function. 
// A better approach without modifying config_unified internals too much:
// We will iterate over the KNOWN KEYS from config_schema_versioning if available, or just call a new helper.

// Let's assume we add `void configUnifiedPrintAll();` to config_unified.h/cpp for the 'dump' command.
// For 'get', we use existing getters.
...
void cmd_config_main(int argc, char** argv) {
    ...
    if (strcmp(argv[1], "get") == 0) cmd_config_get(argc, argv);
    else if (strcmp(argv[1], "set") == 0) cmd_config_set(argc, argv);
    else if (strcmp(argv[1], "dump") == 0) cmd_config_dump(argc, argv);
    else if (strcmp(argv[1], "show") == 0) cmd_config_show(argc, argv);
...
void cmd_config_get(int argc, char** argv) {
    ...
    if (type == NULL) {
        // Fallback: Try to find it by trial or assume int if unknown, but safer to say unknown
        // For flexibility, let's try fetching as int first. If 0 (default), maybe float?
        // Ideally schema knows all.
        Serial.printf("[CONFIG] [WARN] Key '%s' not in schema. Attempting raw fetch...\n", key);
        // Try printing as int
...
void cmd_config_dump(int argc, char** argv) {
    ...
    // Since we don't have a public iterator in this file, we rely on the schema file 
    // or we assume config_unified exposes a print function.
    // To make this self-contained based on previous context, I will create a function 
    // `configUnifiedPrintAll()` in config_unified.cpp (see below) and call it here.
    
    extern void configUnifiedPrintAll(); // You need to add this to config_unified.cpp/h
