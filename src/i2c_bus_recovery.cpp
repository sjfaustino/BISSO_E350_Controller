...
i2c_bus_status_t i2cCheckBusStatus() {
  // Uses system constants
  pinMode(PIN_I2C_SDA, INPUT);
  pinMode(PIN_I2C_SCL, INPUT);
  delay(10);
  bool sda_level = digitalRead(PIN_I2C_SDA);
  bool scl_level = digitalRead(PIN_I2C_SCL);
...
void i2cRecoverBus() {
  ...
  pinMode(PIN_I2C_SDA, OUTPUT); digitalWrite(PIN_I2C_SDA, LOW); delayMicroseconds(5);
  pinMode(PIN_I2C_SCL, OUTPUT); digitalWrite(PIN_I2C_SCL, LOW); delayMicroseconds(5);
  pinMode(PIN_I2C_SCL, INPUT_PULLUP); delayMicroseconds(5);
  pinMode(PIN_I2C_SDA, INPUT_PULLUP); delayMicroseconds(5);
...
i2c_stats_t i2cGetStats() { if (stats.transactions_total > 0) stats.success_rate = (float)stats.transactions_success / stats.transactions_total * 100.0f; return stats; }
...
void i2cMonitorBusHealth() {
  i2c_bus_status_t status = i2cCheckBusStatus();
  if (status != I2C_BUS_OK) {
    faultLogEntry(FAULT_WARNING, FAULT_PLC_COMM_LOSS, -1, status, "I2C Degraded");
  }
}