...
void taskManagerInit() {
  ...
  queue_fault =
      xQueueCreate(QUEUE_LEN_FAULT, QUEUE_ITEM_SIZE); // Size 96 for async logs
  queue_display = xQueueCreate(QUEUE_LEN_DISPLAY, QUEUE_ITEM_SIZE);

  if (!queue_motion || !queue_safety || !queue_encoder || !queue_plc ||
      !queue_fault || !queue_display) {
...
void taskPlcCommCreate() {
  if (xTaskCreatePinnedToCore(taskPlcCommFunction, "PLC_Comm",
                              TASK_STACK_PLC_COMM, NULL, TASK_PRIORITY_PLC_COMM,
                              &task_plc_comm, CORE_1) == pdPASS)
...
void taskShowStats() {
  ...
  for (int i = 0; i < stats_count; i++) {
    ...
    Serial.printf("%-18s %-9lu %-9.2f %-9lu %-6.1f%%\n", task_stats[i].name,
                  task_stats[i].run_count, avg_time,
                  task_stats[i].max_run_time_ms, cpu_percent);
  }
  Serial.println();
}
...
uint8_t taskGetCpuUsage() {
  ...
  if (elapsed < 1000)
    return 0; // Avoid unstable readings at boot

  for (int i = 0; i < stats_count; i++) {
    if (task_stats[i].handle != NULL) {
      total += task_stats[i].total_time_ms;
    }
  }

  // Cap at 100% to handle potential counter rollovers or race conditions
  uint32_t usage = (total * 100) / elapsed;
  return (usage > 100) ? 100 : (uint8_t)usage;
}
