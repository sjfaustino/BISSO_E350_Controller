...
static TaskHandle_t task_telemetry = nullptr; // Background telemetry
static TaskHandle_t task_lcd_formatter = nullptr; // LCD formatting
...
task_stats_t task_stats[] = {
 ...
 {nullptr, "Telemetry", TASK_PRIORITY_TELEMETRY, 0, 0, 0, 0, 0},
...
void taskManagerInit() {
 ...
 // Use sizeof(queue_message_t) to prevent memory corruption
 ...
 // Initialize event groups for event-driven architecture
...
void taskManagerStart() {
 ...
 taskFaultLogCreate();
 taskMonitorCreate();
 taskTelemetryCreate(); // Background telemetry on Core 0
 taskLcdFormatterCreate(); // LCD formatting on Core 0
 taskLcdCreate();
...
// Consolidated I2C Mutexes to prevent bus contention
...
bool taskLockMutex(SemaphoreHandle_t mutex, uint32_t timeout_ms) {
 ...
 // Handle recursive mutexes specifically for motion
...
void taskPlcCommCreate() {
 // Ghost Task Removed (Optimization)
...
void taskTelemetryCreate() { // Background telemetry on Core 0
 if (xTaskCreatePinnedToCore(
...
void taskLcdFormatterCreate() { // LCD formatting on Core 0
...
// ============================================================================
// ADAPTIVE I2C TIMEOUT
// ============================================================================

uint32_t taskGetAdaptiveI2cTimeout() {
 // Scale I2C timeout based on current system load
